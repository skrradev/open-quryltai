...
import {
  ...
  ProgramStatementTypes,
  ProgramTopicMatrix,
  programQueries,
} from '@/entities/program'
import {
  PartyGenderAnalytics,
  PartyCadreAnalytics,
  PartyGeographyAnalytics,
  PartyYouthAnalytics,
  statisticsQueries,
...
export function StatisticsPage() {
  ...
  const genderQuery = useQuery(statisticsQueries.gender(language))
  const youthQuery = useQuery(statisticsQueries.youth(language))
  const cadreQuery = useQuery(statisticsQueries.cadre(language))
  const geographyQuery = useQuery(statisticsQueries.geography(language))
  const programQuery = useQuery(programQueries.comparison(language))
    ...
    <main className="mx-auto min-h-svh w-full max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
      ...
      {statisticsQuery.data && (
        <div className="grid gap-6">
          ...
          {/* Political Parties Showcase Grid */}
          <section aria-labelledby="home-parties-heading" className="space-y-3">
            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-xl font-semibold tracking-tight" id="home-parties-heading">
                  {t('party.showcase.title')}
                </h2>
                <p className="text-xs text-muted-foreground">
                  {t('party.showcase.description')}
                </p>
              </div>
              <Button asChild className="self-start sm:self-auto -ml-3 sm:ml-0" size="sm" variant="ghost">
                <Link to={`/${language}/parties`}>
                  <span>{t('party.showcase.viewAll')}</span>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7">
              {statisticsQuery.data.parties.map(({ party, count }) => (
                <Link
                  className="group relative flex flex-col justify-between rounded-xl border bg-card p-3.5 transition-all hover:border-primary/50 hover:shadow-xs hover:-translate-y-0.5"
                  key={party.id}
                  to={`/${language}/parties/${party.id}`}
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <PartyLogo partyId={party.id} size="sm" />
                      <span
                        className="size-2 rounded-full shrink-0"
                        style={{
                          backgroundColor:
                            PARTY_BRANDING[party.id]?.brandColor ?? 'var(--primary)',
                        }}
                      />
                    </div>
                    <div>
                      <h3 className="font-semibold text-xs leading-snug group-hover:text-primary transition-colors line-clamp-2">
                        {party.name}
                      </h3>
                  ...
                  <div className="mt-2.5 pt-2 border-t border-border/40 flex items-center justify-between text-[10px] font-medium text-muted-foreground group-hover:text-primary transition-colors">
                    <span>{t('actions.viewPartyShort')}</span>
      ...
      {youthQuery.data && (
        <section aria-labelledby="youth-analytics-heading" className="mt-12">
          <PartyYouthAnalytics data={youthQuery.data} />
        </section>
      )}

      {genderQuery.data && (
        <section aria-labelledby="gender-analytics-heading" className="mt-12">
          <PartyGenderAnalytics data={genderQuery.data} youth={youthQuery.data} />
        </section>
      )}

      {cadreQuery.data && (
        <section className="mt-12">
          <PartyCadreAnalytics data={cadreQuery.data} />
        </section>
      )}

      {geographyQuery.data && (
        <section className="mt-12">
          <PartyGeographyAnalytics data={geographyQuery.data} />
        </section>
      )}


      {programQuery.data && (
        <section aria-labelledby="program-heading" className="mt-12 space-y-6">
          <header className="max-w-3xl">
            <h2 className="text-2xl font-semibold tracking-tight" id="program-heading">
              {t('program.title')}
            </h2>
          ...
          <Card>
            <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <CardTitle>{t('program.matrix.title')}</CardTitle>
                <CardDescription>{t('program.matrix.caption')}</CardDescription>
              </div>
              <ProgramMethodologyModal className="shrink-0 self-start" mode="matrix" />
            </CardHeader>
            <CardContent>
              <ProgramTopicMatrix comparison={programQuery.data} language={language} />
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <CardTitle>{t('program.types.title')}</CardTitle>
                <CardDescription>{t('program.types.description')}</CardDescription>
              </div>
              <ProgramMethodologyModal className="shrink-0 self-start" mode="types" />
            </CardHeader>
            <CardContent>
              <ProgramStatementTypes measurements={programQuery.data.parties} />
            </CardContent>
          </Card>
        </section>
      )}
