...
public class CandidateService {
    ...
    public PageDto<CandidateDto> findAll(CandidateQuery query) {
        ...
        List<CandidateDto> items = result.items().stream()
                .map(candidate -> toDto(candidate, false))
                .toList();
    ...
    public CandidateDto findById(short candidateId) {
        return repository.findById(candidateId, localization.currentLanguage())
                .map(candidate -> toDto(candidate, true))
                .orElseThrow(() -> new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "candidate not found"));
    }

    /**
     * @param withFullPhoto details endpoints include the full-size portrait;
     *                      list rows deliberately get the thumbnail only, because
     *                      the originals run to several MB each.
     */
    private CandidateDto toDto(CandidateData candidate, boolean withFullPhoto) {
        return new CandidateDto(
                ...
                candidate.incumbent(),
                assets.url(candidate.photoThumbnailKey()),
                withFullPhoto ? assets.url(candidate.photoKey()) : null,
                withFullPhoto ? candidate.photoWidth() : null,
                withFullPhoto ? candidate.photoHeight() : null);
    }
