...
const candidateSorts = ['listOrder', 'name', 'birthYear', 'party'] as const
...
function parseCandidateListParams(searchParams: URLSearchParams): CandidateListParams {
  ...
  return {
    // Ascending by default: the list is read from № 1 down, which is the order
    // the party published and the order mandates are allocated in. Descending was
    // the default while the list was sorted by a prominence score, where a high
    // value came first; it made the ballot order run backwards.
    ...
    sort: includes(candidateSorts, sort) ? (sort as CandidateSort) : 'listOrder',
  }
}
...
function isDefaultValue(key: string, value: unknown): boolean {
  return (
    (key === 'page' && value === 0) ||
    (key === 'size' && value === 24) ||
    (key === 'sort' && value === 'listOrder') ||
    (key === 'direction' && value === 'asc')
  )
}

export function useCandidateListParams(fixedParams?: Partial<CandidateListParams>) {
  const [searchParams, setSearchParams] = useSearchParams()
  const params = useMemo(() => {
    const parsed = parseCandidateListParams(searchParams)
    return fixedParams ? { ...parsed, ...fixedParams } : parsed
